...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  /* FIXME: Write this */
	...
	if(rightChild < max) { //has two children
		if(_smallerIndexHeap(heap, leftChild, rightChild, compare) == leftChild) {
			smallest = leftChild;
			if(_smallerIndexHeap(heap, pos, smallest, compare) == smallest) {
...
void switcheroo(DynArr *heap, comparator compare) {
	int i, j;
	for(i = 0; i < sizeDynArr(heap); i++) {
		for(j = i + 1; j < sizeDynArr(heap); ++j) {
			if((*compare)(heap->data[i], heap->data[j]) == 1) {
				struct Task *a = heap->data[i];
				heap->data[i] = heap->data[j];
				heap->data[j] = (struct Task *)a;
			}
		}
	}
}
