...
{
    /*FIXME: write this*/
	struct Task *intPtr1, *intPtr2;
	int difference;

	intPtr1 = (struct Task *)left;
	intPtr2 = (struct Task *)right;

	difference = intPtr1->priority - intPtr2->priority;
	if(difference < 0)
		return -1;
...
{
  /*FIXME: Write this */
	TaskP myTask = (TaskP)malloc(sizeof(*myTask));
	myTask->priority = priority;
	strcpy(myTask->description, desc);
