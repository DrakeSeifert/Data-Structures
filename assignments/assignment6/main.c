...
int main (int argc, const char * argv[]) {
	...
	myWord = getWord(fileptr);

	while(myWord != NULL) {
		if(containsKey(hashTable, myWord)) {
			int *value = (int *)atMap(hashTable, myWord);
			...
			*intVal = 1;
			insertMap(hashTable, myWord, intVal);
			free(myWord);
		}
		myWord = getWord(fileptr);
	}
	...
	printKeyValues(hashTable, keyPrint, valPrint); /* Prints all key-value pairs, 
								one pair per line */
...
char* getWord(FILE *file)
{
	...
	{
                character = tolower(character);  /* In should be the same as in 
							..so force all to lowercase */
